...
// GET /api/admin/wallets/balance - Get wallet balance by address
export async function GET(request: NextRequest) {
  try {
    ...
    if (wallet.currency === 'ETH' || wallet.currency === 'BNB') {
      realBalance = await EthereumService.getBalance(address, network);
    } else {
      // ERC20 token balance
      ...
      if (tokenAddress) {
        realBalance = await ERC20Service.getBalance(tokenAddress, address, network);
