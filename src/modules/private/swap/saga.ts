...
// API call function for swap options
function* callSwapOptionsAPI(): Generator<any, any, any> {
  const response = yield call(fetch, '/api/swap/options', {
    method: 'GET',
    headers: {
...
// API call function for swap history
function* callSwapHistoryAPI(telegramId: string): Generator<any, any, any> {
  const response = yield call(fetch, `/api/swap/history?telegramId=${telegramId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
...
function* fetchSwapOptionsSaga(action: FetchSwapOptionsRequestAction): Generator<Effect, void, any> {
  try {
    const response = yield call(callSwapOptionsAPI);
    ...
    if (data.success) {
      yield put(fetchSwapOptionsSuccess(data.swapOptions));
    } else {
      yield put(fetchSwapOptionsFailure(data.message || 'Failed to fetch swap options'));
    }
  } catch (error: any) {
    console.error('Fetch swap options error:', error);
    yield put(fetchSwapOptionsFailure('Failed to fetch swap options'));
  }
}

// Fetch swap history saga
function* fetchSwapHistorySaga(action: FetchSwapHistoryRequestAction): Generator<Effect, void, any> {
  try {
    const { telegramId } = action.payload;
    const response = yield call(callSwapHistoryAPI, telegramId);
    ...
    if (data.success) {
      yield put(fetchSwapHistorySuccess(data.transactions));
    } else {
      yield put(fetchSwapHistoryFailure(data.message || 'Failed to fetch swap history'));
    }
  } catch (error: any) {
    console.error('Fetch swap history error:', error);
    yield put(fetchSwapHistoryFailure('Failed to fetch swap history'));
  }
}
