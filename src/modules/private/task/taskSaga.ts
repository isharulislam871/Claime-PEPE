...
function* completeTaskSaga(action: ReturnType<typeof completeTaskRequest>): Generator<Effect, void, unknown> {
  try {
    ...
    const { hash , signature ,  timestamp } =  generateSignature(JSON.stringify({
      telegramId: currentUser.telegramId,
      taskId,
      reward: task.reward,
      taskType: task.type
    }) , process.env.NEXT_PUBLIC_APP_SECRET || 'app')
 
    const { response , status  } : any = yield call(API_CALL, { url : '/users/tasks'  , body : { hash , signature ,  timestamp }  , method : 'POST'})  
