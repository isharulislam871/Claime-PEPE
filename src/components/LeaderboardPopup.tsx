...
import {
  selectCurrentUser,
} from '@/modules/private/user/selectors';
import {
...
import { closePopup, selectIsLeaderboardPopupVisible } from '@/modules';
...
const LeaderboardPopup = ( ) => {
  ...
  const handleTabChange = (newTab: 'weekly' | 'monthly' | 'all-time') => {
    ...
    if (user?.telegramId) {
      dispatch(fetchLeaderboardRequest(newTab, user.telegramId, 10));
    }
  };
const onClose = () => {
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <button
                onClick={() => {
                  if (user?.telegramId) {
                    dispatch(fetchLeaderboardRequest(activeTab, user.telegramId, 10));
                  }
                }}
