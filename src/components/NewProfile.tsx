...
export default function NewProfile({ isOpen, onClose }: NewProfileProps) {
    ...
    try {
      ...
      setTimeout(() => {
        setUserStats({
          ...
          tasksCompleted: user?.tasksCompletedToday || 0,
          adsWatched: 0,/* user?.totalAdsViewed */
          referrals: user?.referralCount || 0,
          joinDate: user?.createdAt || new Date().toISOString(),
          level: Math.floor((user?.totalEarned || 0) / 10000) + 1
        });
        setLoading(false);
      }, 1000);
    } catch (error) {
