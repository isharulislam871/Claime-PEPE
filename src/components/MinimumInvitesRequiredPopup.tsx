...
export default function MinimumInvitesRequiredPopup({ 
  ...
  requiredInvites,
  onInviteFriends 
}: MinimumInvitesRequiredPopupProps) {
  const remainingInvites = Math.max(0, requiredInvites - currentInvites);
  const progress = Math.min(100, (currentInvites / requiredInvites) * 100);
  const isEligible = currentInvites >= requiredInvites;
